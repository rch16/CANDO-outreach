...
float pulseWidth = 0.01; // pulse width in ms
...
void desynchronised(){
  ...
  currentDirection = checkDirection(i,1); // check direction
...
int checkDirection(int i, int mode){
  
  int h = 1;
  int currentDirection;

  if(mode == 1){
    if((ys[i+h]-ys[i])/h >= 0) currentDirection = 1;
    else currentDirection = 0;
  }

  else if(mode == 1){
    if((yn[i+h]-yn[i])/h >= 0) currentDirection = 1;
    else currentDirection = 0;
  }
