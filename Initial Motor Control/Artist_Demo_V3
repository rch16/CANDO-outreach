// double check with tim & francesca
// how to make less of a jump from end of acceleration to beginning of constant
...
char auth[] = "5iCA25psPcDwtnSFWXKdKvwON5A65APz";
...
int accelSteps = 1500;
float constPulseWidth = 0.01;
float pulseWidth = 0.0025; // pulse width in ms
float rpm = 150; // revolutions per minute -> increasing this increases the speed
int inputSteps = 500; // number of steps for delay increment/decrement
float constMinDelay = 100L * 1000L / (0.5*totalSteps) / rpm;
float minDelay = 100L * 1000L / totalSteps / rpm; // minimum delay in ms
float constDelay = 100L * 1000L / totalSteps / 100;
float maxDelay = 10*minDelay; // maximum delay in ms
float delayValue = (maxDelay - minDelay)/totalSteps; // delay increment/decrement value
...
void loop() {
  ...
  if(go == 1){
    if(buttonState == 0){
      ...
      smoothMotion(accelSteps,totalSteps);
      changeDirection();
    }
  }*/
  smoothMotion(accelSteps,totalSteps);
...
void desynchronised(){
  ...
  currentDirection = checkDirection(i); // check direction
  if(previousDirection == currentDirection) steps(n); // direction hasn't changed
  else {
    decelerate(50);
    digitalWrite(directionPin,currentDirection);
    accelerate(50);
...
void steps(int n){
  ...
  for(int i = 1; i <= n; i++){
    digitalWrite(stepPin, HIGH);
    delay(constPulseWidth);
    digitalWrite(stepPin, LOW);
    delay(constMinDelay - pulseWidth);
    // no change in the delay means rotational speed remains the same / constant
  }
}

int checkDirection(int i){
  int h = 1;
