...
// authorisation token for blynk app
char auth[] = "2JSoPiQdWlss5KwoDA2ZHNMldihjHt5q";
...
BLYNK_WRITE(V1){
  // controlling number of steps
  inputSteps = param.asInt();
}

BLYNK_WRITE(V2) {
  // controlling up or down motion
  int directionButton = param.asInt();
  if (directionButton == 0) {
    // up
    terminal.println("up selected.");
    digitalWrite(directionPin, LOW);
  }
  else{
    // down
    terminal.println("down selected.");
...
BLYNK_WRITE(V3) {
  // controlling smooth or sinusoidal motion
  int motionButton = param.asInt();
  if (motionButton == 0) {
    // sinusoidal
    terminal.println("sinusoidal motion selected.");
    motion = "sinusoid";
  }
  else{
    // constant
    terminal.println("constant motion selected.");
    motion = "constant";
  }
  terminal.flush(); // Ensure everything is sent
}
...
void controlMotor(){
  ...
  int n = inputSteps; // number of steps

  if(motion == "sinusoid"){
    // mode a -> slow
    terminal.print(motion);
    terminal.println(" motion starting...");
    terminal.flush();
    accelerate(n);
    steps(n);
    decelerate(n);
    terminal.print(motion);
    terminal.println(" motion complete.");
  }
  else if(motion == "constant"){
    // mode b -> fast    
    terminal.print(motion);
    terminal.println(" motion starting...");
    terminal.flush();
...
void accelerate(int n){
  // motor actuation code for acceleration
  ...
  for(int i = 1; i <= n; i++){
    ...
    delay(pulseWidth);
    digitalWrite(stepPin, LOW);
    delay(del - pulseWidth);

    // gradually decreasing the delay increases the rotational speed
    del = del - delayValue;
  }
}

void decelerate(int n){
  // motor actuation code for deceleration
  ...
  for(int i = 1; i <= n; i++){
    ...
    // gradually increasing the delay decreases the rotational speed
    del = del + delayValue;
  }
}
...
void steps(int n){
  ...
  for(int i = 1; i <= n; i++){
    ...
    delay(pulseWidth);
    digitalWrite(stepPin, LOW);
    delay(minDelay - pulseWidth);
    // no change in the delay means rotational speed remains the same / constant
  }
}
